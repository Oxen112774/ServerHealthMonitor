...
type Config struct {
	...
	MaxRestarts       int // maximum automatic restarts per instance in the rolling window
	RestartWindow     int // rolling window for MaxRestarts, in seconds
...
// Validate checks safety and consistency constraints before startup.
func (c *Config) Validate() error {
	if !isLoopbackHost(c.Host) && !c.TLSEnabled {
		return fmt.Errorf("non-loopback agent listeners require tls_enabled; basic auth over HTTP is not allowed")
	}
	if c.Port < 1 || c.Port > 65535 {
		return fmt.Errorf("port must be between 1 and 65535")
	}

	if c.ConnectivityPort < 1 || c.ConnectivityPort > 65535 {
		return fmt.Errorf("connectivity_port must be between 1 and 65535")
	}
	if c.CollectInterval <= 0 || c.CheckInterval < 3 {
		return fmt.Errorf("collect_interval must be positive and check_interval must be at least 3 seconds")
	}
	if c.MaxAlerts <= 0 || c.FailureThreshold <= 0 || c.RestartCooldown < 0 || c.MaxRestarts <= 0 || c.RestartWindow <= 0 || c.SocketWaitTimeout <= 0 || c.NotifyCooldown < 0 {
		return fmt.Errorf("alert and recovery limits contain invalid values")
	}
	if c.WarnCPU < 0 || c.WarnCPU > 100 || c.WarnMemory < 0 || c.WarnMemory > 100 || c.WarnDisk < 0 || c.WarnDisk > 100 || c.WarnLoad < 0 || c.WarnProcessCount < 0 {
		return fmt.Errorf("resource thresholds contain invalid values")
	}
	if (c.AuthUser == "") != (c.AuthPass == "") {
		return fmt.Errorf("auth_user and auth_pass must be configured together")
	}
	if c.MetricsAuth && (c.AuthUser == "" || c.AuthPass == "") {
		return fmt.Errorf("metrics_auth requires auth_user and auth_pass")
	}
	...
	if c.AnomalyEnabled && (c.AnomalyWindowSize < 2 || c.AnomalyZThreshold <= 0 || c.AnomalyEWMAAlpha <= 0 || c.AnomalyEWMAAlpha > 1) {
		return fmt.Errorf("anomaly detection parameters are invalid")
	}
	if c.AlertGroupWindow <= 0 || c.AlertEscalateAfter <= 0 {
		return fmt.Errorf("alert management intervals must be positive")
	}
...
func Load(path string) (*Config, error) {
	...
	for scanner.Scan() {
		...
		case "max_restarts":
			cfg.MaxRestarts = parseInt(v, cfg.MaxRestarts)
		case "restart_window":
			cfg.RestartWindow = parseInt(v, cfg.RestartWindow)
