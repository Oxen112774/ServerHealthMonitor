...
func TestPasswordChangeInvalidatesExistingSessionTokens(t *testing.T) {
	a := NewAuthManager(t.TempDir())
	apiKey, err := a.CreateUser("alice", "Strong-pass-123", RoleAdmin)
	if err != nil {
		t.Fatalf("create user: %v", err)
	...
	login := a.Login("alice", "Strong-pass-123", apiKey)
	if !login.Success {
		t.Fatalf("first login should succeed: %+v", login)
	}
	...
	if _, _, ok := a.ValidateToken(oldToken); ok {
		t.Fatal("old session token must be invalid after password change")
	}

	newLogin := a.Login("alice", "New-pass-456", apiKey)
	if !newLogin.Success {
		t.Fatalf("login with new password should succeed: %+v", newLogin)
	}
	if _, _, ok := a.ValidateToken(newLogin.Token); !ok {
		t.Fatal("new session token must validate")
	}
}
