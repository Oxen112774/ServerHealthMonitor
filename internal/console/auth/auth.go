...
type User struct {
	...
	TokenVersion      int64        `json:"token_version"`       // increments whenever password or role changes; invalidates old sessions
}
...
// CanManageUser reports whether an actor may administer a target user.
// The role assignment policy also prevents lower-privileged administrators
// from operating on owners or directors.
func CanManageUser(actor, target UserRole) bool {
...
func (a *AuthManager) DeleteUser(username string) error {
	...
	if user.Role == RoleOwner {
		...
		for _, candidate := range a.users {
			if candidate.Role == RoleOwner {
				ownerCount++
			}
		}
		if ownerCount <= 1 {
			return errors.New("不能删除最后一个服主")
		}
	}
...
func (a *AuthManager) Login(username, password, apiKey string) LoginResult {
	...
	// Generate token
	token := GenerateToken(username, string(u.Role), u.TokenVersion, a.TokenTTL)
...
func GenerateToken(username, role string, tokenVersion int64, ttl time.Duration) string {
	// Simple token: base64(payload).hash
	// For production, use proper JWT. This is lightweight.
	expiry := time.Now().Add(ttl).Unix()
	payload := username + "|" + role + "|" + itoa(tokenVersion) + "|" + itoa(expiry)
...
func ValidateToken(token string) (string, string, bool) {
	username, role, _, ok := parseTokenClaims(token)
	return username, role, ok
}

func (a *AuthManager) ValidateToken(token string) (string, string, bool) {
	username, role, version, ok := parseTokenClaims(token)
	if !ok {
		return "", "", false
	}
...
func parseTokenClaims(token string) (string, string, int64, bool) {
	...
	username := fields[0]
	role := fields[1]
	tokenVersion := atoi(fields[2])
	expiry := atoi(fields[3])
	if time.Now().Unix() > int64(expiry) {
		return "", "", 0, false
	}
	return username, role, int64(tokenVersion), true
}
