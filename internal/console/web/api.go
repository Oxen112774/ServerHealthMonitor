...
func (h *Handler) handleServersAPI(w http.ResponseWriter, r *http.Request) {
	username := r.Header.Get("X-Username")
	ip := h.getClientIP(r)
	if r.Method != http.MethodGet && !h.hasPermission(r, auth.PermissionManageServers) {
	...
	case http.MethodGet:
		...
		for _, server := range servers {
			item := map[string]interface{}{"id": server.ID, "name": server.Name, "host": server.Host, "port": server.Port, "user": server.User, "agent_port": server.AgentPort, "status": server.Status, "group_ids": server.GroupIDs, "last_check": server.LastCheck, "created_at": server.CreatedAt}
			if h.hasPermission(r, auth.PermissionViewSensitive) {
...
func (h *Handler) handleTicketsAPI(w http.ResponseWriter, r *http.Request) {
	...
	switch r.Method {
	case http.MethodGet:
		h.jsonOK(w, h.tickets.List(username, h.hasPermission(r, auth.PermissionManageTickets)))
	case http.MethodPost:
		var req struct {
...
func (h *Handler) handleTicketDetailAPI(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodPut || !h.hasPermission(r, auth.PermissionManageTickets) {
...
func (h *Handler) handleDeployAPI(w http.ResponseWriter, r *http.Request) {
	...
	if req.ServerID != "" {
		req.ServerIDs = append(req.ServerIDs, req.ServerID)
	}
	if req.GroupID != "" {
		for _, s := range h.servers.ListServers() {
			for _, gid := range s.GroupIDs {
				if gid == req.GroupID {
					req.ServerIDs = append(req.ServerIDs, s.ID)
					break
				}
			}
		}
	}
	ids := uniqueStrings(req.ServerIDs)
	if len(ids) == 0 {
	...
	job, err := h.servers.StartDeployment(ids, username, servers.DeployOptions{
		BinaryPath: req.BinaryPath, SSHPassword: req.SSHPassword, Adapter: req.Adapter,
		RollbackOnFailure: rollback, MaxConcurrent: req.MaxConcurrent,
	})
	if err != nil {
		h.audit.Log(username, audit.ActionDeploy, strings.Join(ids, ","), "部署任务创建失败: "+err.Error(), ip, false)
		h.jsonError(w, err.Error(), http.StatusBadRequest)
		return
	}

	h.audit.Log(username, audit.ActionDeploy, strings.Join(ids, ","), "创建部署任务", ip, true)
	h.jsonOK(w, job)
}

func (h *Handler) handleDeploymentsAPI(w http.ResponseWriter, r *http.Request) {
	if r.Method != http.MethodGet {
		h.jsonError(w, "方法不允许", http.StatusMethodNotAllowed)
		return
	}
	h.jsonOK(w, h.servers.ListDeployments(100))
}

func (h *Handler) handleDeploymentDetailAPI(w http.ResponseWriter, r *http.Request) {
	id := strings.TrimPrefix(r.URL.Path, "/console/api/deployments/")
	if r.Method == http.MethodPost {
		var req struct {
		...
		if err := h.servers.CancelDeployment(id); err != nil {
			h.jsonError(w, err.Error(), http.StatusBadRequest)
			return
		}
		h.jsonOK(w, nil)
	...
	job, ok := h.servers.GetDeployment(id)
	if !ok {
		h.jsonError(w, "部署任务不存在", http.StatusNotFound)
		return
	}
	h.jsonOK(w, job)
}

func (h *Handler) handleGroupsAPI(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		h.jsonOK(w, h.servers.ListGroups())
	case http.MethodPost:
		var req struct {
		...
		group, err := h.servers.AddGroup(req.Name, req.Description)
		if err != nil {
			h.jsonError(w, err.Error(), http.StatusBadRequest)
			return
		}
		for _, id := range uniqueStrings(req.ServerIDs) {
			if err := h.servers.AssignGroup(id, group.ID, true); err != nil {
				h.jsonError(w, err.Error(), http.StatusBadRequest)
				return
			}
		}
		h.jsonOK(w, group)
	case http.MethodDelete:
		id := strings.TrimPrefix(r.URL.Path, "/console/api/groups/")
		if err := h.servers.DeleteGroup(id); err != nil {
			h.jsonError(w, err.Error(), http.StatusBadRequest)
			return
		}
		h.jsonOK(w, nil)
...
func (h *Handler) handleUsersAPI(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPost:
		...
		actorRole := auth.UserRole(r.Header.Get("X-Role"))
		if !auth.ValidRole(requestedRole) {
			requestedRole = auth.RoleViewer
		}
		if !auth.CanManageUser(actorRole, requestedRole) {
			h.jsonError(w, "无权创建该角色的用户", http.StatusForbidden)
			return
		}
...
func (h *Handler) handleUserDetailAPI(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodDelete:
		...
		target, ok := h.auth.GetUser(targetUser)
		if !ok {
			h.jsonError(w, "用户不存在", http.StatusNotFound)
			return
		}
		if !auth.CanManageUser(auth.UserRole(r.Header.Get("X-Role")), target.Role) {
			h.jsonError(w, "无权操作该角色的用户", http.StatusForbidden)
			return
		}
