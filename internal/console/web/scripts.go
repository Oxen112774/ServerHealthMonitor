...
function openDeploy(id) {
	...
	document.getElementById('deploy-group-id').value = '';
...
async function doDeploy() {
	...
	const groupId = document.getElementById('deploy-group-id').value.trim();
	const maxConcurrent = parseInt(document.getElementById('deploy-concurrency').value, 10) || 4;
	...
	if (res.ok && res.data.success) {
		...
		output.textContent = '任务已加入队列: ' + jobId;
		await waitDeployment(jobId, output, btn);
	} else {
	...
	async function waitDeployment(id, output, btn) {
		for (let i = 0; i < 600; i++) {
			await new Promise(function(resolve) { setTimeout(resolve, 1000); });
			const res = await apiCall('/console/api/deployments/' + id);
			if (!res.ok || !res.data.success) continue;
			const job = res.data.data;
			output.textContent = job.status + '\n' + (job.output || '');
			if (['succeeded', 'failed', 'rolled_back', 'cancelled'].indexOf(job.status) >= 0) {
				btn.disabled = false;
				btn.textContent = job.status === 'succeeded' ? '部署完成' : '重试';
				toast(job.status === 'succeeded' ? '部署成功' : '部署未完成', job.status === 'succeeded' ? 'success' : 'error');
				return;
			}
		}
		btn.disabled = false;
		btn.textContent = '重试';
		toast('任务状态查询超时', 'error');
	}
}
