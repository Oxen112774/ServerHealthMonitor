...
// The current sample is evaluated against the historical window before it is
// appended, so a sudden deviation cannot inflate its own baseline.
func (d *Detector) Evaluate(value float64) Result {
	...
	// Not enough historical samples yet — return baseline info without flagging.
	if len(d.window) < d.minSamples {
		d.appendSample(value)
		res.Reason = "warming_up"
		return res
	}

	// Compute the baseline without including the sample being evaluated.
	...
	if stddev < 1e-9 {
		...
		if deviation > 1e-9 {
			...
			// Small drift from a constant baseline is distinct from a
			// materially higher/lower spike.
			if deviation <= math.Max(1, math.Abs(mean)*0.1) {
...
func (d *Detector) appendSample(value float64) {
