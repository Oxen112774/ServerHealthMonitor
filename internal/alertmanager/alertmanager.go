...
type Config struct {
	...
	// TickInterval controls background processing frequency. It is capped
	// relative to the group window so short windows are dispatched promptly.
...
func NewManager(cfg Config, dispatch DispatchFunc) *Manager {
	...
	if shortInterval := cfg.GroupWindow / 2; shortInterval > 0 && shortInterval < cfg.TickInterval {
		cfg.TickInterval = shortInterval
	}
