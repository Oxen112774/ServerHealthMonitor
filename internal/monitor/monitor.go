...
type InstanceState struct {
	...
	LastRestartTime int64   `json:"last_restart_time"` // unix
	LastState       string  `json:"last_state"`
	RestartTimes    []int64 `json:"restart_times,omitempty"` // unix timestamps in the rolling window
	CircuitOpen     bool    `json:"circuit_open"`
	CircuitOpenedAt int64   `json:"circuit_opened_at,omitempty"` // unix
}
...
type Monitor struct {
	...
	maxRestarts       int
	restartWindow     int
	restartFunc       func(collector.Instance)
...
// SetRestartPolicy configures the per-instance rolling restart limit.
func (m *Monitor) SetRestartPolicy(maxRestarts, windowSeconds int) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if maxRestarts > 0 {
	...
	if windowSeconds > 0 {
		m.restartWindow = windowSeconds
	}
}

// States returns a snapshot of remediation state for all known instances.
func (m *Monitor) States() map[string]InstanceState {
	m.mu.Lock()
	defer m.mu.Unlock()
	out := make(map[string]InstanceState, len(m.states))
	for service, state := range m.states {
		copyState := *state
		copyState.RestartTimes = append([]int64(nil), state.RestartTimes...)
		out[service] = copyState
	}
	return out
}
...
func (m *Monitor) ClearCircuit(service string) bool {
	...
	for name, state := range m.states {
		...
		if state.CircuitOpen || len(state.RestartTimes) > 0 {
			cleared = true
		}
...
func (m *Monitor) checkInstance(inst collector.Instance, now int64) {
	...
	if isUnhealthy {
		...
		if st.FailureCount >= m.failureThreshold {
			m.pruneRestartTimes(st, now)
			if st.CircuitOpen {
				log.Printf("[WARN] %s 自动修复熔断已打开，暂停自动重启", inst.Service)
				st.LastState = inst.State
				return
			}
			if len(st.RestartTimes) >= m.maxRestarts {
				st.CircuitOpen = true
				st.CircuitOpenedAt = now
				log.Printf("[WARN] %s 在 %d 秒内自动重启达到 %d 次，打开熔断",
					inst.Service, m.restartWindow, m.maxRestarts)
				m.notifier.Push(notifier.Alert{
					Time:  ts,
					Type:  notifier.SeverityCritical,
					Title: fmt.Sprintf("自动修复熔断 - %s", inst.Service),
			...
			canRestart := st.LastRestartTime == 0 ||
				(now-st.LastRestartTime) >= int64(m.restartCooldown)
			if canRestart {
				st.LastRestartTime = now
				st.RestartTimes = append(st.RestartTimes, now)
				st.FailureCount = 0
				restart := m.restartFunc
				if restart == nil {
					restart = m.restartService
				}
				go restart(inst)
			} else {
...
func (m *Monitor) pruneRestartTimes(st *InstanceState, now int64) {
	cutoff := now - int64(m.restartWindow)
	kept := st.RestartTimes[:0]
	for _, timestamp := range st.RestartTimes {
		if timestamp > cutoff {
			kept = append(kept, timestamp)
		}
	}
	st.RestartTimes = kept
}
